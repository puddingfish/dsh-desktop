...
模型路由的核心思想：**按任务类型把请求分流到不同档位的模型**——杂活走便宜模型，硬活走旗舰，在不牺牲体验的前提下把费用降下来。先理解四个角色：

| 角色 | 负责什么 | 选型原则 |
|---|---|---|
| **主会话** | 你直接对话的模型：理解需求、规划、写关键代码 | 综合能力优先——它决定体验下限 |
| **子代理** | 后台委派任务（research、独立小任务） | 便宜 + 快即可；**调用频率最高，是省钱大头** |
| **轻量任务** | 命中关键词的杂活（总结/翻译/格式化…） | 全线最便宜档 |
| **重型任务** | 命中关键词的硬活（重构/架构/调试…） | 旗舰档，只在需要时出场 |

> 下拉选项 = 你在 DSH 中已配置的模型。先把模型加进 DSH（设置 → 模型），再来「模型路由」页分配角色。

### 按需求选一套

| 方案 | 主会话 | 子代理 | 轻量 | 重型 | 适合谁 |
|---|---|---|---|---|---|
| ⚖️ **均衡省钱**（推荐起步） | 标准档 | flash 档 | flash 档 | 旗舰档 | 大多数人：日常够用，硬活上旗舰 |
| 💰 **预算优先** | 标准档 | flash 档 | flash 档 | 留空 | 轻度使用，成本压到最低 |
| 🚀 **性能优先** | 旗舰档 | 标准档 | flash 档 | 旗舰档 | 重度编码、质量第一 |
| 🪶 **极简尝鲜** | flash 档 | flash 档 | flash 档 | 标准档 | 简单场景先跑起来再说 |

> 示例（按你实际配置的模型替换）：主会话 `GLM-5.3`，子代理/轻量 `DeepSeek-v4-flash`，重型 `KIMI-K3`。

### 配置五步

1. 侧边栏打开「模型路由」→ 打开「启用路由」开关；
2. 按上表给各角色选 Provider / 模型（**留空 = 不改写**，安全默认）；
3. 轻量关键词建议：`总结, 翻译, 格式化, 改名, 写文档, 列大纲`；
4. 重型关键词建议：`重构, 架构, 性能, 优化, 调试, 疑难`；
5. 保存 → 看 dsh web 控制台，出现 `model-router: … A/B -> C/D (light: keyword "总结")` 即生效。

### 边界（知道这些更踏实）

- 你在会话里**手动切过模型**的请求不会被覆盖（「尊重显式选择」默认开）；
- 路由器自身异常自动回退原配置，绝不阻断请求；
- 上下文压缩、会话标题生成有各自的模型设置（`~/.dsh/settings.yaml`），不走此路由——同样可以单独设成便宜模型，进一步省钱。

完整配置项语义与已知边界见 [插件 README](plugins/dsh-model-router/README.zh.md)。
