 ...
 * GET /mmx-vision/status 返回 mmx 软检测状态（休眠时客户端据此隐藏全部入口）。
 ...
 * GET /mmx-vision/status 返回 mmx 软检测状态（客户端据此决定是否显示
 * 设置卡片与图片改写；休眠时全部隐藏，且本路由始终可用）。
 * @param ctx - 插件上下文。
 * @param readMaxBytes - 每请求字节上限读取器。
 * @param readAvailability - mmx 可用性读取器（启动时定格）。
 */
export function registerAttachRoute(ctx, readMaxBytes = () => DEFAULT_MAX_BYTES, readAvailability = () => ({ available: true })) {
  ...
  webserver.register({
    ...
    handler: async (req, res) => {
      ...
      if (req.method === 'GET' && pathname === '/mmx-vision/status') {
        json(res, { ok: true, value: readAvailability() })
        return
      }
      if (req.method === 'GET') {
