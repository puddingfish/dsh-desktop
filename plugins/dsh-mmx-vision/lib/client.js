 ...
 * - 软检测：启动时问宿主 GET /mmx-vision/status，本机没有 mmx CLI 时
 *   上述 UI 一概不装（静默休眠，零报错）。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region mmx soft-detect
		async function fetchMmxAvailability() {
			try {
				const response = await fetch("/mmx-vision/status", { headers: { accept: "application/json" } });
				if (!response.ok) return true;
				const envelope = await response.json();
				const value = envelope && typeof envelope === "object" ? envelope.value : void 0;
		...
		function apply(ctx) {
			...
			// mmx 软检测：宿主报告本机没有 mmx CLI 时整体静默休眠——
			// 发送改写、缩略图、设置卡片一概不装，也不产生任何报错
			// （检测失败则保持启用：宁可多显示，不可误隐藏）。
			void (async () => {
				const available = await fetchMmxAvailability();
				if (!available) return;
				try {
