 ...
 * - 软检测：本机找不到 mmx CLI（也没显式配置 mmxPath）时整体静默休眠——
 *   不注册工具、不打告警，客户端隐藏设置卡片与图片改写（GET /mmx-vision/status）；
...
import { callMmxVision, createVisionCache, mmxBinaryOnPath, resolveImage, resolveMmxEntry } from './mmx-client.js'
...
/**
 * mmx 可用性检测（启动时定格，装好 mmx 后重启生效）。
 * - 显式配置了 mmxPath → 激活（信任用户意图，路径错时调用才给可操作指引）；
 * - 自动发现 npm 全局 mmx-cli，或 PATH 上有 mmx 可执行 → 激活；
 * - 都没有 → 休眠：不注册工具、不告警、客户端隐藏全部入口。
 * @param {object} spec - 已解析配置。
 * @returns {{ available: boolean, mode: 'configured' | 'discovered' | 'path' | 'absent' }}
 */
export function computeAvailability(spec) {
  if (spec.mmxPath !== undefined) return { available: true, mode: 'configured' }
  if (resolveMmxEntry(undefined) !== undefined) return { available: true, mode: 'discovered' }
  if (mmxBinaryOnPath() !== undefined) return { available: true, mode: 'path' }
  return { available: false, mode: 'absent' }
}
...
export function apply(ctx, config = {}) {
  ...
  // mmx 软检测（启动时定格）：本机有 mmx → 插件激活；没有 → 整体静默休眠：
  // 不注册 describe_image、不打任何告警，客户端经 GET /mmx-vision/status
  // 感知后隐藏设置卡片与图片改写——对没有 MiniMax 订阅的用户零打扰。
  const availability = computeAvailability(spec())
  // webserver 可选：headless 部署没有 webserver，仅工具可用。fiber inject——
  // 服务就绪（或重组后再次就绪）时挂路由，服务消失时自动卸载。
  ctx.inject(['webServer'], (wctx) => {
    registerAttachRoute(wctx, () => spec().maxBytes, () => availability)
  })

  if (availability.available) ctx.tools.register(
    defineTool({
      name: 'describe_image',
